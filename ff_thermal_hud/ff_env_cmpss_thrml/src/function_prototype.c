...
/* MLX90640 */
#define IIC_MLX_DEVICE_ID		XPAR_IIC_3_DEVICE_ID
...
/* MLX90640 */
#define IIC_MLX_INTR_ID	XPAR_INTC_0_IIC_3_VEC_ID

#define INTC			    XIntc
#define INTC_HANDLER		XIntc_InterruptHandler

/* MLX90640 */
#define MLX_ADDRESS 		0x33
...
u8 MlxIicAddr;		/* Variable for storing MLX90640 IIC address */
...
extern int InitIICMlx90640()
{
  ...
  MlxIicAddr = MLX_ADDRESS;


  /*
   * Initialize the MLX IIC so that it is ready to use.
   */
  ConfigPtr = XIic_LookupConfig(IIC_MLX_DEVICE_ID);
  if (ConfigPtr == NULL) {
    return XST_FAILURE;
  }

  Status = XIic_CfgInitialize(&IicMlx, ConfigPtr, ConfigPtr->BaseAddress);
  if (Status != XST_SUCCESS) {
	return XST_FAILURE;
  }
...
extern int SetupMlxInterruptSystem(XIic *IicMlxInstPtr)
{
  ...
  Status = XIntc_Initialize(&IntcMlx, INTC_DEVICE_ID);
   ...
   */
  Status = XIntc_Connect(&IntcMlx, IIC_MLX_INTR_ID,
   ...
   */
  Status = XIntc_Start(&IntcMlx, XIN_REAL_MODE);
  if (Status != XST_SUCCESS) {
    return XST_FAILURE;
  }
   ...
   */
  XIntc_Enable(&IntcMlx, IIC_MLX_INTR_ID);
  ...
  Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_INT,
			 (Xil_ExceptionHandler)INTC_HANDLER, &IntcMlx);
...
extern void SetupIICMlxIntrHandlers() {
   ...
   */
  XIic_SetSendHandler(&IicMlx, &IicMlx, (XIic_Handler) SendHandler);
  XIic_SetRecvHandler(&IicMlx, &IicMlx, (XIic_Handler) ReceiveHandler);
  XIic_SetStatusHandler(&IicMlx, &IicMlx,
...
extern int SetMlxIICAddress() {
   ...
   */
  Status = XIic_SetAddress(&IicMlx, XII_ADDR_TO_SEND_TYPE, MlxIicAddr);
  if (Status != XST_SUCCESS) {
	return XST_FAILURE;
  }
...
extern int MLX90640_I2CRead(uint8_t slaveAddr,uint16_t startAddress, uint16_t nMemAddressRead, uint16_t *data)
{
	 ...
	 */
    IicMlx.Options = XII_REPEATED_START_OPTION;

	TransmitComplete = 1;

    //xil_printf("MasterSend\r\n");
	XIic_MasterSend(&IicMlx, cmd,2);
	...
	XIic_MasterRecv(&IicMlx, i2cData,2*nMemAddressRead);
...
extern int MLX90640_I2CWrite(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
	 ...
	 * Send the data.
	 */
	Status = XIic_MasterSend(&IicMlx, cmd, 4);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
